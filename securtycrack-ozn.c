/*
 * 802.11 WEP / WPA-PSK Anahtar Kırıcı

 *
 * Bu program ücretsiz bir yazılımdır; yeniden dağıtabilir ve/veya değiştirebilirsiniz
 * tarafından yayınlanan GNU Genel Kamu Lisansı koşulları altındadır.
 * Özgür Yazılım Vakfı; Lisansın 2. versiyonu veya
 * (seçeneğinize bağlı olarak) daha sonraki herhangi bir sürüm.
 *
 * Bu program faydalı olması ümidiyle dağıtılmaktadır,
 *ancak HERHANGİ BİR GARANTİ OLMAKSIZIN; zımni garantisi bile olmadan
 * TİCARİ ELVERİŞLİLİK veya BELİRLİ BİR AMACA UYGUNLUK.  Bkz.
 ...
 *
 *
 * Ayrıca özel bir istisna olarak telif hakkı sahipleri
 * bu programın bazı bölümlerinin kodunu
 * Her birinde açıklandığı gibi belirli koşullar altında OpenSSL kitaplığı
 * bireysel kaynak dosyası ve bağlantılı kombinasyonları dağıtma
 *ikisi dahil.
 * Her bakımdan GNU Genel Kamu Lisansına uymalısınız
 * OpenSSL dışında kullanılan tüm kodlar için. * Eğer değiştirirseniz
 * dosya(lar)ı bu istisna dışında, bu istisnayı kendi
 * dosya(lar)ın sürümü, ancak bunu yapmak zorunda değilsiniz. * Eğer
 * bunu yapmak istemiyorsanız, bu istisna bildirimini hesabınızdan silin.
 * versiyon. * Bu istisna bildirimini tüm kaynaklardan silerseniz
 * programdaki dosyaları silin, ardından onu da buradan silin.
 */
...
#include "securtycrack-ozn/defs.h"
#include "securtycrack-ozn/ce-wpa/crypto_engine.h"
#include "securtycrack-ozn/securtycrack-ozn.h"
#include "securtycrack-oznn/osdep/byteorder.h"
#include "radiotap/platform.h"
#include "securtycrack-ozn/adt/avl_tree.h"
#include "securtycrack-ozn/support/common.h"
#include "securtycrack-ozn/tui/console.h"
#include "securtycrack-ozn/cpu/cpuset.h"
#include "securtycrack-ozn/support/crypto_engine_loader.h"
#include "securtycrack-ozn/cpu/simd_cpuid.h"
#include "securtycrack-ozn/cpu/trampoline.h"
#include "securtycrack-ozn/cowpatty/cowpatty.h"
#include securtycrack-ozn/crypto/crypto.h"
#include "securtycrack-ozn/third-party/hashcat.h"
#include "linecount.h"
#include "securtycrack-ozn/support/pcap_local.h"
#include "session.h"
#include "securtycrack-ozn/ce-wep/uniqueiv.h"
#include "securtycrack-ozn/version.h"
#include "wkp-frame.h"
#include "securtycrack-ozn/osdep/osdep.h"
#include "securtycrack-ozn/third-party/ieee80211.h"
#include "securtycrack-ozn/third-party/ethernet.h"
...
static const char usage[]
	= "\n"
	  "\n"
	  "  usage: securtycrack-ozn [options] <input file(s)>\n"
	  "\n"
	  "  Common options:\n"
	  "\n"
...
int main(int argc, char * argv[])
{
	...
	progname = getVersion(
		"securtycrack-ozn", _MAJ, _MIN, _SUB_MIN, _REVISION, _BETA, _RC);
